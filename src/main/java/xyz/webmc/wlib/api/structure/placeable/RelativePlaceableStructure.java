...
public class RelativePlaceableStructure extends AbstractPlaceableStructureBase<RelativeBlock> {
  ...
  public void place(Location loc, Object chunk) {
    for (final RelativeBlock block : this.blocks) {
      ...
      if (isInChunk(blockLocation, chunk)) {
        block.place(loc, chunk);
      }
    }
  }

  @Override
  public final void loadSchematic(InputStream stream, int offsetx, int offsety, int offsetz) {
    try (stream) {
