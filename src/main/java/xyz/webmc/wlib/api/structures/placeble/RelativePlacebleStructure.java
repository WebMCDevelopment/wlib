...
public class RelativePlacebleStructure extends PlacebleStructure<RelativeBlock> {
    public RelativePlacebleStructure() {
        super(new ArrayList<>());
    }
    ...
    public void place(Location loc, Chunk chunk) {
        for (final RelativeBlock block : this.blocks) {
            ...
            if (blockLocation.getWorld() == chunk.getWorld()
                    && blockLocation.getBlockX() >> 4 == chunk.getX()
                    && blockLocation.getBlockZ() >> 4 == chunk.getZ()) {
                block.place(loc);
            }
        }
    }
    ...
    @Override
    public void loadSchematic(InputStream stream, int offsetx, int offsety, int offsetz) throws SchemLoadingError {
        try (stream) {
                ...
                for (int z = 0; z < schematic.length(); z++) {
                    for (int x = 0; x < schematic.width(); x++) {
                        ...
                        if (block != SchematicBlock.AIR) {
                            final XMaterial material = XMaterial.matchXMaterial(block.block().replace("minecraft:", ""))
